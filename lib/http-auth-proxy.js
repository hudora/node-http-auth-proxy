...
ProxyServer.prototype.startServer = function() {
  ...
  console.log('started server on port ' + host + ':' + port);
};
...
ProxyServer.prototype.handleRequest = function(hostReq, hostResp) {
  ...
  console.log('got a new request: ' + hostReq.method + ' ' + hostReq.url);
  ...
  if(!authCookie) {
    ...
    if(!this.isAuthValid(auth)) {
      var thisHost = hostReq.headers.host,
      redirectUrl = querystring.escape('http://' + thisHost + hostReq.url),
      authUrl = this.config.authUrl + '?redirection=' + redirectUrl;
      console.log('redirecting to ' + authUrl + ' because of: ' + auth);
...
ProxyServer.prototype.checkForAndValidateMagicToken = function(req) {
  ...
  var requestedUrl = 'http://' + req.headers.host + req.url.split('?')[0],
      hmac = crypto.createHmac('sha1', this.config.magicTokenKey || '???'),
      hash = hmac.update(requestedUrl),
      digest = hmac.digest('base64');

  // did we find such a hash value? If not just bail out
